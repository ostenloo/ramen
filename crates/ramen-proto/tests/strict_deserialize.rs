...
//!
//! A corpus case pins one input; this test pins the *class* (the mechanical check the `Denial` sweep asked for). Every type the
...
//!   (`OpResult` → `WhoamiResult` / `FileWriteResult`);
//! - a transparent newtype's inner type decides (`SessionId`).
//!
